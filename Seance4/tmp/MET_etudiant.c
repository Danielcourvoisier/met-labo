...
void InitialisationEtudiant()
{
	...
	thetaMoteur_old = 0.0;
	// courant
	Kp_i = 19.26; //pour marge phase = 45
	Gi_i = 1/0.001896;  //pour marge phase = 45
	// vitesse
	Kp_w = .0;
	Gi_w = .0;
	// position
...
RapportsCycliques Regulation(FlagsRegulation* flags, float consigne,float courantBranche1, float courantBranche2, float thetaMoteur, float busDC)
{
	...
	**------------------------------------------------------------------------------------------*/
	...
	if(flags->bit.NewPosition && flags->bit.ChoixCapteur == RESOLVER)
	{
		...
		{
			nombreTours++;
			delta_theta+=2*PI; // modif 2 pour éviter dérivation d'échelon (pente "infinie")
			
		}
		if(delta_theta > PI)
		{
			nombreTours--;
			delta_theta-=2*PI; // modif 2
		}
		...
		// Calcul vitesse (modif 1)
		wf = alpha*w +(1-alpha)*wf; // vitesse filtrée (modif 3)
		w = delta_theta*Fe2; 		// vitesse non filtrée
	}
	...
	**------------------------------------------------------------------------------------------*/
		    ...
		    //Axe Q: Calcul de UQC
		    Kpeiq=(iqc-iqm)*Kp_i;
			int_iq=int_iq+(Gi_i*h)*(Kpeiq-((uqcp-uqc))); //Flag anti windup removed
			uqcp=Kpeiq+int_iq;
			...
			//Axe D: Calcul de UDC
			Kpeid=(idc-idm)*Kp_i;
			int_id=int_id+(Gi_i*h)*(Kpeid-((udcp-udc))); //Flag anti windup removed
			udcp=Kpeid+int_id;
